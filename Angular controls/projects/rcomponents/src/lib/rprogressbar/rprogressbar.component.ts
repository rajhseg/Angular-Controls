...
export class RProgressbarComponent extends RBaseComponent<any> implements AfterViewInit, AfterViewChecked {
  ...
  private _type: RProgressBarType = RProgressBarType.Infinite;

  allProgressBarTypes = RProgressBarType;
  allDisplayTypes = RProgressBarDisplayType;

  @Input()
  set Type(val: RProgressBarType) {
    this._type = val;
    this.isTypeInit = true;

    if (this.DisplayType == RProgressBarDisplayType.StraightLine && val == RProgressBarType.Progress) {
      this.RenderStraightLine();
    }


    if (this.DisplayType == RProgressBarDisplayType.Circle && val == RProgressBarType.Progress) {
      this.RenderProgressCircle();
    }
  }
  get Type(): RProgressBarType {
    return this._type;
  }

  private _displayType: RProgressBarDisplayType = RProgressBarDisplayType.Circle;

  private _width: string = '';

  @Input()
  set DisplayType(val: RProgressBarDisplayType) {
    this._displayType = val;
    this.isTypeInit = true;

    if (val == RProgressBarDisplayType.StraightLine && this.Type == RProgressBarType.Progress) {
      this.RenderStraightLine();
    }


    if (val == RProgressBarDisplayType.Circle && this.Type == RProgressBarType.Progress) {
      this.RenderProgressCircle();
    }
  }
  get DisplayType(): RProgressBarDisplayType {
  ...
  RenderProgressCircle() {
    if (this.progressCanvas) {
      ...
      if (this.context == null && this.winobj.isExecuteInBrowser()) {
        this.context = this.progressCanvas?.nativeElement.getContext('2d');
      }
