...
export class RCalenderComponent extends RBaseComponent<Date> implements IDropDown, OnInit, AfterViewInit, OnDestroy, ControlValueAccessor, IPopupCloseInterface {
  ...
  LoadMonth(date: Date, isSelect: boolean = true) {
    ...
    for (let i = firstdayInWeek - 1; i >= 1; i--) {
      ...
      firstWeekDays.push(new Day(prevMonth.getDate(), prevMonth.getDay(), prevMonth.getMonth(), prevMonth.getFullYear()));
    }

    for (let i = 1; i <= firstWeekRemainingDays; i++) {
      let curMonth = new Date(year, monthinNumber, i);
      let a = new Day(curMonth.getDate(), curMonth.getDay(), curMonth.getMonth(), curMonth.getFullYear());
